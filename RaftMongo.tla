...
\*
\* We define actions in this spec that match the protocol in different versions
\* of MongoDB. Comment out the commit point learning actions except for
\* AdvanceCommitPoint, LearnCommitPointWithTermCheckAction, and
\* LearnCommitPointFromSyncSourceNeverBeyondLastAppliedAction to match MongoDB
\* 4.2, the latest version at the time of this writing.
Next ==
    ...
    \* \/ LearnCommitPointAction \* repros a bug: learning commit point from heartbeat is unsafe
    \* \/ LearnCommitPointFromSyncSourceAction \* repros 2 later bugs: one liveness, one in 5-node sets
    \* \/ AppendEntryAndLearnCommitPointFromSyncSourceAction \* repros a bug after a long trace
    \/ LearnCommitPointWithTermCheckAction \* safe, but contains misbehavior (keeps too much history in memory)
    \/ LearnCommitPointFromSyncSourceNeverBeyondLastAppliedAction \* truly correct
